...
def get_mean(features, labels):
    ...
    global_mean = 0. * torch.mean(class_mean, dim=0)
...
def maximal_angle(features, labels, weights, global_mean=None, class_mean=None):
    ...
    res_h = torch.mean((nrm_h / denom_h)[mask_h != 0])
    ...
    res_w = torch.mean((nrm_w / denom_w)[mask_w != 0])
...
def class_mean_convergence(features, labels, weights, global_mean=None, class_mean=None):
    ...
    W_tilde = weights / torch.norm(weights)

    return torch.norm(W_tilde - M_tilde) ** 2
