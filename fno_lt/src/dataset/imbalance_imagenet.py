...
class ImageNetLT(DefaultDataset):
    num_classes = 1000

    def __init__(self, cfg, train=True, class_map=None, **kwargs):
        super(ImageNetLT, self).__init__(cfg, train=train)
        ...
        if self.train:
            ...
            self.txt = os.path.join(cfg.dataset.root, 'data_txt/ImageNet_LT_train.txt')
        else:
            self.transform = transforms.Compose([
            ...
            self.txt = os.path.join(cfg.dataset.root, 'data_txt/ImageNet_LT_test.txt')

        self.img_path = []
        self.targets = []
        with open(self.txt) as f:
            for line in f:
                self.img_path.append(os.path.join(cfg.dataset.root, 'imagenet', line.split()[0]))
